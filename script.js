...
save.addEventListener("click", function (e) {
  ...
  fetch("http://localhost/E-sisda_skema_editor/save_file.php", {
    method: "POST",
    // headers:{
  ...
  if (target.classList.contains("bahan")) {
    ...
    popup.addEventListener("click", (pop) => {
      ...
      if (pop.target.classList.contains("resize")) {
        ...
        console.log(popup);
        let leftPopup = popup.style.left.replace("px", "");
        modal.style.top = pop.clientY + "px";
        modal.style.left = 200 + Number(leftPopup) + "px";
        modal.style.zIndex = 9;
        // if (target.classList.contains("panah")) {
        //   document.getElementById("inp-height").setAttribute("disabled", true);
        // } else {
        //   document.getElementById("inp-height").setAttribute("disabled", false);
        // }
        const inpResize = document.getElementsByClassName("inpResize");

        for (let i = 0; i < inpResize.length; i++) {
          const element = inpResize[i];
          element.addEventListener("change", (c) => {
            console.log(c);
            if (c.target.id === "inp-height")
              target.style.height = c.target.value + "px";
            if (c.target.id === "inp-width")
              target.style.width = c.target.value + "px";
          });
        }
        // target = null;
